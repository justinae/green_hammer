...
class newsStory{
	...
	function show(){
		if($this->title){
			if($this->link){
				echo "<div class=\"announcement\"><h4 class=\"no_margin\"><a href=\"$this->link\" target=\"_blank\">$this->title</a></h4>\n";
			}elseif($this->title){
				echo "<div class=\"announcement\"><h4 class=\"no_margin\">$this->title</a></h4>\n";
			}
			echo "<p class=\"no_margin\">";
			if($this->description)
				preg_match("/^(\S+\s+){0,20}/", "$this->description", $matches);
				echo trim($matches[0]) . '...';
